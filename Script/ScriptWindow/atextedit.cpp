...
//    ScriptLanguage(lang), lHelp(labelHelpTooltip), TabInSpaces(A3Global::getInstance().TabInSpaces)
    ScriptLanguage(lang), lHelp(labelHelpTooltip), TabInSpaces(LocalTabInSpaces)
{
...
{
     return SaveTextToFile(document()->toPlainText(), fileName);
}
