...
AConfig_SI::AConfig_SI() :
    ScriptHub(AScriptHub::getInstance()), Config(MasterConfig::getInstance())
{
...
bool AConfig_SI::load(QString FileName)
{
    ...
    QString err = ScriptHub.loadConfig(FileName);
...
{
    QString err = ScriptHub.saveConfig(FileName);
    if (err.isEmpty()) return true;
...
{
    return ScriptHub.JSON.toVariantMap();
}
...
bool AConfig_SI::setConfig(QVariantMap ConfigObject)
{
    ...
    QString err = ScriptHub.loadConfig(json);
    if (err.isEmpty()) return true;
...
bool AConfig_SI::replace(QString Key, QVariant Value)
{
    ...
    bool ok = modifyJsonValue(ScriptHub.JSON, Key, jv);
    if (ok)
    {
        ScriptHub.loadConfig(ScriptHub.JSON);
        return true;
    }
...
QVariant AConfig_SI::getKeyValue(QString Key)
{
    ...
    QJsonObject obj = ScriptHub.JSON;
    int indexOfDot;
    QString path = Key;
    do
    {
...
int AConfig_SI::countLogicalChannels() const
{
    return Config.CountLogicalChannels();
}

bool AConfig_SI::isNegativeHardwareChannel(int iHardwChannel)
{
    return Config.IsNegativeHardwareChannel(iHardwChannel);
}

#include "channelmapper.h"
bool AConfig_SI::isNegativeLogicalChannel(int iLogicalChannel)
{
    const int iHardwCh = Config.Map->LogicalToHardware(iLogicalChannel);
    return Config.IsNegativeHardwareChannel(iHardwCh);
}

bool AConfig_SI::isIgnoredHardwareChannel(int iHardwChannel)
{
    return Config.IsIgnoredHardwareChannel(iHardwChannel);
}

bool AConfig_SI::isIgnoredLogicalChannel(int iLogicalChannel)
{
    const int iHardwCh = Config.Map->LogicalToHardware(iLogicalChannel);
    return Config.IsIgnoredHardwareChannel(iHardwCh);
}

int AConfig_SI::toHardware(int iLogicalChannel)
{
...
int AConfig_SI::toLogical(int iHardwChannel)
{
