...
{
    const QList<QScreen*> listScr = qApp->screens();
...
QString guitools::dialogSaveFile(QWidget * parent, const QString & text, const QString & filePattern)
{
    ...
    QString fileName = QFileDialog::getSaveFileName(parent, text, LastSaveDir, filePattern);
...
QString guitools::dialogLoadFile(QWidget *parent, const QString &text, const QString &filePattern)
{
    ...
    QString fileName = QFileDialog::getOpenFileName(parent, text, LastLoadDir, filePattern);
    if (fileName.isEmpty()) return "";
    //GlobSet.LastLoadDir = QFileInfo(fileName).absolutePath();
    LastLoadDir = QFileInfo(fileName).absolutePath();
    return fileName;
}
...
QString guitools::dialogDirectory(QWidget * parent, const QString & text, const QString & initialDir, bool DefaultRead, bool DefaultWrite)
{
    ...
    if (DefaultRead)  LastLoadDir = dir;
    //if (DefaultWrite) GlobSet.LastSaveDir = dir;
    if (DefaultWrite) LastSaveDir = dir;
    return dir;
}
...
{
    QString str;
    bool ok = parseJson(json, name, str);
...
{
    bool flag;
    bool ok = parseJson(json, name, flag);
...
{
    int num;
    bool ok = parseJson(json, name, num);
